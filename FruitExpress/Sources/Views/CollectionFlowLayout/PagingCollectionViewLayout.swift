...
final class PagingCollectionViewLayout: UICollectionViewFlowLayout {
    ...
    override func targetContentOffset(
        ...
        guard speed != 0 else {
            if scrollDirection == .horizontal {
                return .init(x: currentPage * pageLength, y: 0)
            } else {
                return .init(x: 0, y: currentPage * pageLength)
            }
        }
        ...
        if scrollDirection == .horizontal {
            return .init(x: nextPage * pageLength, y: 0)
        } else {
            return .init(x: 0, y: nextPage * pageLength)
        }
    }
}
